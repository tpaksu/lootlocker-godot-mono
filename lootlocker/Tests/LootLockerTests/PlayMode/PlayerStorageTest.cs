...
namespace LootLockerTests.PlayMode
{
    public class PlayerStorageTest
    {
        ...
        public IEnumerator PlayerStorage_CreatePayload_SucceedsAndReturnsStorage()
        {
            ...
            Assert.That(actualResponse.payload.Length, Is.EqualTo(actualRequest.payload.Count), "Payload was not of the expected size");
            ...
            foreach (var expectedKVPair in actualRequest.payload)
            {
                foreach (var actualKVPair in actualResponse.payload)
                {
                    ...
                    {
                        matchedKVPairs++;
                        Assert.That(actualKVPair.value, Is.EqualTo(expectedKVPair.value), $"Value did not match for key {expectedKVPair.key}");
                        Assert.That(actualKVPair.is_public, Is.EqualTo(expectedKVPair.is_public), $"Is Public flag did not match for key {expectedKVPair.key}");
                    }
                }
            }
            Assert.That(matchedKVPairs, Is.EqualTo(actualRequest.payload.Count), "Not all storage kv pairs were matched in the response");
        }
        ...
        public IEnumerator PlayerStorage_UpdatePayload_Succeeds()
        {
            ...
            Assert.That(initialResponse.payload.Length, Is.EqualTo(actualRequest.payload.Count), "Payload did not contain the expected number of items");
            ...
            foreach (var expectedKVPair in actualRequest.payload)
            {
                foreach (var actualKVPair in actualResponse.payload)
                {
                    ...
                    {
                        keysMatched++;
                        Assert.That(actualKVPair.is_public, Is.Not.EqualTo(expectedKVPair.is_public), "Is Public flag was not flipped");
                        Assert.That(actualKVPair.value, Is.Not.EqualTo(expectedKVPair.value), "Value did not change from creation");
                        foreach (var updatedKVPair in updateRequest.payload)
                        {
                            if (updatedKVPair.key.Equals(expectedKVPair.key))
                            {
                                Assert.That(actualKVPair.value, Is.EqualTo(updatedKVPair.value), "Value was not what the update request set");
                            }
                        }
                    }
                }
            }
            Assert.That(keysMatched, Is.EqualTo(actualRequest.payload.Count), "Not all keys were found in fetched payload");
        }
            ...
            foreach (var expectedKVPair in actualRequest.payload)
            {
                if (expectedKVPair.is_public)
                {
                    ...
                    foreach (var actualKVPairs in actualResponse.payload)
                    {
                        ...
                        {
                            kvPairsMatched++;
                            Assert.That(actualKVPairs.value, Is.EqualTo(expectedKVPair.value), "Value as gotten by player 2 was not as set by player 1");
                        }
                    }
                }
            }
            Assert.That(actualResponse.payload.Length, Is.EqualTo(publicKVPairCount), "Player 2's received storage length did not match the amount of public keys in player 1's storage");
            Assert.That(actualResponse.payload.Length, Is.EqualTo(kvPairsMatched), "Not all received keys matched with the expected keys");
        }
    }
}
